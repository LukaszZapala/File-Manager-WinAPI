...
Workspace::Workspace() : hComboBox(NULL), hListView(NULL), hText(NULL), isUsedNow(FALSE)
{
...
BOOL Workspace::CreateComboBox(HWND hWnd, UINT uId)
{
	...
	if (hComboBox == NULL) return FALSE;
	else return TRUE;
}
...
void Workspace::ComboBoxInit()
{
	...
	int length = ComboBox_GetTextLength(hComboBox);
	LPWSTR string = (LPWSTR)GlobalAlloc(GPTR, length + 1);
...
void Workspace::CreateImageList()
{
	...
	HBITMAP hBmp = NULL;
	// 0. File icon
	hBmp = LoadBitmap(GetModuleHandle(NULL), MAKEINTRESOURCE(IDB_FILE));
	ImageList_Add(hImageList, hBmp, (HBITMAP)NULL);
	DeleteObject(hBmp);
	// 1. Folder icon
	hBmp = LoadBitmap(GetModuleHandle(NULL), MAKEINTRESOURCE(IDB_FOLDER));
	ImageList_Add(hImageList, hBmp, (HBITMAP)NULL);
	DeleteObject(hBmp);
	// 2. Folder open icon
	hBmp = LoadBitmap(GetModuleHandle(NULL), MAKEINTRESOURCE(IDB_FOLDER_OPEN));
	ImageList_Add(hImageList, hBmp, (HBITMAP)NULL);
	DeleteObject(hBmp);
	// 3. File exe icon
	hBmp = LoadBitmap(GetModuleHandle(NULL), MAKEINTRESOURCE(IDB_EXE));
	ImageList_Add(hImageList, hBmp, (HBITMAP)NULL);
	DeleteObject(hBmp);
	// 4. File audio type icon
	hBmp = LoadBitmap(GetModuleHandle(NULL), MAKEINTRESOURCE(IDB_AUDIO));
	ImageList_Add(hImageList, hBmp, (HBITMAP)NULL);
	DeleteObject(hBmp);
	// 5. File video type icon
	hBmp = LoadBitmap(GetModuleHandle(NULL), MAKEINTRESOURCE(IDB_VIDEO));
	ImageList_Add(hImageList, hBmp, (HBITMAP)NULL);
	DeleteObject(hBmp);
	// 6. File Image type icon
	hBmp = LoadBitmap(GetModuleHandle(NULL), MAKEINTRESOURCE(IDB_IMAGE));
	ImageList_Add(hImageList, hBmp, (HBITMAP)NULL);
	DeleteObject(hBmp);
	// 7. File document type icon
	hBmp = LoadBitmap(GetModuleHandle(NULL), MAKEINTRESOURCE(IDB_DOCUMENT));
	ImageList_Add(hImageList, hBmp, (HBITMAP)NULL);
	DeleteObject(hBmp);
...
void Workspace::UpdateList(TCHAR * _path)
{
	...
	int i = 0;
	for (i = 0; FindNextFile(hFile, &data); i++)
	{
	...
	if (i == 0)
	{
		if (MessageBox(NULL, TEXT("Access denied!"), TEXT("Error"), MB_OK | MB_ICONERROR) == IDOK)
			UpdateList(tOldPath);
	}
...
BOOL Workspace::Initialization(HWND hWnd, UINT uComboId, UINT uListId, UINT uTextId, BOOL isLeft)
{
	...
	{
		MessageBox(NULL, TEXT("Function CreateComboBox failure!"), TEXT("Error"), MB_OK | MB_ICONERROR);
		return FALSE;
	}
	ComboBoxInit();
	...
	{
		MessageBox(NULL, TEXT("Function CreateEditText failure!"), TEXT("Error"), MB_OK | MB_ICONERROR);
		return FALSE;
	}
